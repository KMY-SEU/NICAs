...
def m_dm(dm, d):
    ...
    m = np.log(dm * (d - 1) + d) / np.log(
        d) - 1  # the formula for the inversion d_m --> m follows from the geometric sum.
    return int(round(m))
...
def coeffextract(t, q, d):
    ...
    return c  # returns the tensor of c
...
def sigcumulextract(X, winlen, W):  # (this function is needed for (meta) detail-checks only)
    ...
    esig_X = esig_X.unsqueeze(
        0)  # signatory needs the tensor esig_X to be 2-dimensional, dims corr. to (batch,channels)
    ...
    logEsig_X = signatory.signature_to_logsignature(esig_X, d, m,
                                                    mode='expand')  # --> torch.tensor of shape(N',), ind.~(9)
...
def SigIC(X, mu, winlen):
    ...
    for k in range(d - 1):
        ...
        for j in range(int(round(mu[k]))):
            W[k] = W[k] + crosscumu(u, v,
                                    j + 1)  # assemble W_k(u_k,v_k) = \sqcup_{j=1,...,mu[k]} crosscumu(u,v,j) (cf. (4))
...
def SigCF(X, mu, winlen):
    ...
    # (X), no gradients, s = torch.zeros(d - 1)
    s = []
    ...
    for k in range(d - 1):
        ...
        norms = dict.fromkeys(
            [tuple(q) for q in W[k]])  # allocate dictionary (with q \in W[k] as keys, i.e.: norms = {q: None})
        ...
        for q in W[k]:
            ...
            # (x) no gradients，normfactors_1 = torch.tensor(
            #     [coeffextract(logESX, [f[0] + f[0]], d) ** (-len(f) / 2) for f in normfactors_0])

            normfactors_1 = torch.stack(
                [coeffextract(logESX, [f[0] + f[0]], d) ** (-len(f) / 2) for f in normfactors_0])
        ...
        # (x) no gradients
        # s[k] = torch.sum(
        #     torch.tensor([(norms[tuple(q)] * coeffextract(logESX, q, d)) ** 2 for q in W[k]]))  # k-th inner sum in (13)

        s += [torch.sum(
            torch.stack([(norms[tuple(q)] * coeffextract(logESX, q, d)) ** 2 for q in W[k]]))]  # k-th inner sum in (13)

    # Compute \Phi_mu(X) = \sum_{k=0}^{d-2} s[k] (cf. (6))
    SigICX = torch.sum(torch.stack(s))
